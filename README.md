...
Prueba automática de extremo a extremo: `CramionEditor.exe --selftest <carpeta> <modelo> <hdr> [imagen]` (incluye un escenario de física en Play). Pruebas sin GPU: `build\CramionCore\CramionPhysicsTests.exe`, `CramionTerrainTests.exe`, `CramionMaterialTests.exe`, `CramionCinematicsTests.exe`, `CramionCoreTests.exe`.
